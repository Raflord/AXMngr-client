...
import { z } from "zod";
import { DeleteRecord, fetchFiltered } from "../api/api";
...
function Remove() {
  ...
  const { mutate: deleteRecord } = useMutation({
    ...
    mutationFn: async (inputData: { id: string }) => {
      return await DeleteRecord(inputData);
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["getFiltered"] });
    },
    onError: (error) => {
      alert(
...
export const Route = createLazyFileRoute("/remove")({
  component: Remove,
});
